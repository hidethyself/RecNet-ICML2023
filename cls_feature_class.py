...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        self.no_channel_to_dop = params['no_channel_to_dop']
        self.pct = params['pct']
        self.corelated_channel = params["corelated_channel"]
    ...
    def data_doping(self, audio):
        channels_to_dop = random.sample(list(range(self._nb_channels)), self.experiment[0])
        channels_to_drop = random.sample(list(range(self._nb_channels)), self._nb_channels - self.experiment[0])
        if self.experiment[1] == 0:
            for channel in channels_to_dop:
                audio[:, channel] = self._eps
        elif self.experiment[1] == 1:
            for channel in channels_to_dop:
                audio[:, channel] = np.random.normal(0, 1, audio.shape[0])
        elif self.experiment[1] == 2:
            audio = audio[:, channels_to_drop]
        elif self.experiment[1] == 3:
            audio = self.dopping_audio_pct(audio=audio, pct=self.pct, no_dopped_channel=self.no_channel_to_dop)
        return audio
    ...
    def preprocess_features(self):
        ...
        if self._is_eval:
            ...
            if self.scaler_type == 0:
                spec_scaler = preprocessing.StandardScaler()
            elif self.scaler_type == 1:
                spec_scaler = preprocessing.MinMaxScaler()
            else:
                print("Wrong choice of scaler.")
                exit()
